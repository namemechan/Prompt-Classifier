...
2.  **분류 옵션 설정**:
    ...
    *   **`안전 모드`**: 파일을 즉시 이동하는 대신, 먼저 대상 위치로 **복사**합니다. 복사가 완료되면, 원본과 복사본의 파일 수와 용량을 비교하여 일치하는지 확인합니다. 확인 후 사용자에게 팝업창을 띄워 원본 파일을 **[삭제]**, **[보존]**, 또는 작업을 **[실행 취소]**(복사본 삭제)할지 선택하도록 합니다. 데이터 유실을 방지하는 가장 안전한 방법입니다.
...
3.  **분류 시작**:
    ...
    *   진행 상황은 `진행 상태 표시줄`과 `로그` 영역에서 실시간으로 확인할 수 있습니다. 완료 시 로그에는 총 소요 시간과 함께 처리된 이미지 개수가 표시됩니다.
...
### v1.0.8
*   기능 추가: 안전모드 및 복제모드 추가
*   버그 수정: nai이미지 분류 버그 수정, 프롬프트 분류 시 특수기호가 들어간 프롬프트일때 발생하는 버그수정
